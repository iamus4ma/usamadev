...
function Reply({ topic }) {
  switch (topic) {
    case 'about': return <><ResponseText as="h2">A little about Usama.</ResponseText><ResponseText>I’m Usama Hassan, a full stack developer. I build responsive web applications, pairing clean frontend design with the backend architecture that makes it all work.</ResponseText><ResponseText>My work spans messaging apps, real estate platforms, booking experiences, and tools that make day-to-day operations easier.</ResponseText><div className="reply-note"><Icon name="code" /><span>Frontend detail. Full stack thinking.</span></div></>;
    ...
    case 'skills': return <><ResponseText as="h2">The tools behind the work.</ResponseText><ResponseText>JavaScript is at the heart of my stack. I use React on the frontend, with Node.js, Express, and MongoDB behind it.</ResponseText><div className="skill-groups">{skillGroups.map(group => <section key={group.title}><h3>{group.title}</h3><ul className="tech-tags">{group.skills.map(skill => <li key={skill}>{skill}</li>)}</ul></section>)}</div></>;
    case 'experience': return <><ResponseText as="h2">My journey so far.</ResponseText><ResponseText>From university projects to professional frontend development.</ResponseText><Timeline items={experience} /><h3 className="reply-subheading">Education</h3><Timeline items={education} /></>;
    case 'services': return <><ResponseText as="h2">Let’s build something useful.</ResponseText><ResponseText>I help take web products from the interface through to the backend.</ResponseText><dl className="conversation-services"><dt>Web development</dt><dd>End-to-end web applications, backend architecture, and APIs.</dd><dt>Frontend development</dt><dd>Responsive interfaces and React-based frontend architecture.</dd><dt>UI/UX design</dt><dd>Intuitive interfaces, product design, and prototyping.</dd></dl><a className="conversation-link" href="mailto:usama.0.vip@gmail.com">Tell me about your project<Icon name="external" /></a></>;
    ...
    case 'contact': return <><ResponseText as="h2">Start a real conversation.</ResponseText><ResponseText>Have a project in mind, a role to discuss, or just want to say hello? Get in touch directly.</ResponseText><a className="contact-email" href="mailto:usama.0.vip@gmail.com">usama.0.vip@gmail.com<Icon name="external" /></a><div className="contact-socials"><ExternalLink href="https://www.linkedin.com/in/usama-hassan-383b2b227/">LinkedIn</ExternalLink><ExternalLink href="https://github.com/iamus4ma">GitHub</ExternalLink></div></>;
    default: return <><ResponseText as="h2">Let’s stay with what I know.</ResponseText><ResponseText>This is a portfolio with prepared replies, so I don’t have an answer to that question. Try asking about my projects, tech stack, experience, services, résumé, or contact details.</ResponseText></>;
  }
}
