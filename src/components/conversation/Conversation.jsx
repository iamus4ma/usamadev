...
import { education, experience, matchTopic, skillGroups, topics } from './content';
...
function Timeline({ items }) {
  return <ol className="conversation-timeline">{items.map(item => <li key={item.place}><div><h4>{item.role}</h4><p>{item.place}</p></div><span>{item.period}</span></li>)}</ol>;
}

function Reply({ topic }) {
  switch (topic) {
    case 'about': return <><h2>A little about Usama.</h2><p>I’m Usama Hassan, a full stack developer. I build responsive web applications, pairing clean frontend design with the backend architecture that makes it all work.</p><p>My work spans messaging apps, real estate platforms, booking experiences, and tools that make day-to-day operations easier.</p><div className="reply-note"><Icon name="code" /><span>Frontend detail. Full stack thinking.</span></div></>;
    case 'projects': return <><h2>Ideas turned into working products.</h2><p>Here are {projects.length} projects from my portfolio, built around real use cases.</p><div className="conversation-projects">{[...projects].reverse().map((project, index) => <article className="conversation-project" key={project.id}><span className="project-number">{String(index + 1).padStart(2, '0')}</span><div><div className="project-heading"><h3>{project.title}</h3><span>{project.period}</span></div><p>{project.description}</p><ul className="tech-tags" aria-label={`${project.title} technologies`}>{project.technologies.map(tech => <li key={tech}>{tech}</li>)}</ul><div className="project-links">{project.demo !== '#' && <ExternalLink href={project.demo}>View project</ExternalLink>}{project.github !== '#' && <ExternalLink href={project.github}>Source code</ExternalLink>}</div></div></article>)}</div></>;
    case 'skills': return <><h2>The tools behind the work.</h2><p>JavaScript is at the heart of my stack. I use React on the frontend, with Node.js, Express, and MongoDB behind it.</p><div className="skill-groups">{skillGroups.map(group => <section key={group.title}><h3>{group.title}</h3><ul className="tech-tags">{group.skills.map(skill => <li key={skill}>{skill}</li>)}</ul></section>)}</div></>;
    case 'experience': return <><h2>My journey so far.</h2><p>From university projects to professional frontend development.</p><Timeline items={experience} /><h3 className="reply-subheading">Education</h3><Timeline items={education} /></>;
    case 'services': return <><h2>Let’s build something useful.</h2><p>I help take web products from the interface through to the backend.</p><dl className="conversation-services"><dt>Web development</dt><dd>End-to-end web applications, backend architecture, and APIs.</dd><dt>Frontend development</dt><dd>Responsive interfaces and React-based frontend architecture.</dd><dt>UI/UX design</dt><dd>Intuitive interfaces, product design, and prototyping.</dd></dl><a className="conversation-link" href="mailto:usama.0.vip@gmail.com">Tell me about your project<Icon name="external" /></a></>;
    case 'testimonials': return <><h2>A few words from clients.</h2><div className="conversation-quotes">{testimonials.map(item => <figure key={item.id}><blockquote>“{item.description}”</blockquote><figcaption><img src={item.image} alt="" width="32" height="32" loading="lazy" />{item.title}</figcaption></figure>)}</div></>;
    case 'resume': return <><h2>Take the details with you.</h2><p>Download my résumé for a closer look at my background and experience.</p><a className="resume-download" href={resume} download="Usama-Hassan-Resume.pdf"><Icon name="file" /><span><strong>Usama Hassan</strong><small>Résumé · PDF</small></span><Icon name="download" /></a></>;
    case 'contact': return <><h2>Start a real conversation.</h2><p>Have a project in mind, a role to discuss, or just want to say hello? Get in touch directly.</p><a className="contact-email" href="mailto:usama.0.vip@gmail.com">usama.0.vip@gmail.com<Icon name="external" /></a><div className="contact-socials"><ExternalLink href="https://www.linkedin.com/in/usama-hassan-383b2b227/">LinkedIn</ExternalLink><ExternalLink href="https://github.com/iamus4ma">GitHub</ExternalLink></div></>;
    default: return <><h2>Let’s stay with what I know.</h2><p>This is a portfolio with prepared replies, so I don’t have an answer to that question. Try asking about my projects, tech stack, experience, services, résumé, or contact details.</p></>;
  }
}
...
export default function Conversation() {
  ...
  return <div className="conversation-app" data-chat-theme={dark ? 'dark' : 'light'} onKeyDown={handleNavigationKeys}>
    ...
    <div className="conversation-workspace">
      ...
      <main ref={mainRef} id="conversation-main" className={`conversation-main ${messages.length ? 'has-messages' : ''}`} tabIndex="-1">
        {!messages.length ? <section className="conversation-welcome"><div className="welcome-avatar"><img src={portrait} alt="" width="64" height="64" /><span><Icon name="code" /></span></div><p className="welcome-intro">A different way to get to know me.</p><h1>Hi, I’m Usama.<br /><span>What would you like to explore?</span></h1><p className="welcome-description">Full stack developer. Thoughtful interfaces.<br className="desktop-break" /> Web applications that work from front to back.</p><div className="suggested-prompts">{['projects', 'skills', 'about', 'contact'].map(id => { const topic = topics.find(item => item.id === id); return <button key={id} onClick={() => ask(topic.prompt, id)}><Icon name={topic.icon} /><strong>{topic.prompt}</strong><span>{topic.hint}</span><Icon name="external" className="prompt-arrow" /></button>; })}</div></section> : <div className="conversation-thread" role="log" aria-label="Portfolio conversation" aria-live="polite" aria-relevant="additions">{messages.map((message, index) => <section className="conversation-turn" key={index} ref={index === messages.length - 1 ? latestRef : null}><div className="visitor-message"><span className="sr-only">You asked: </span>{message.prompt}</div><div className="portfolio-message"><div className="reply-author"><span className="reply-monogram">u.</span><strong>Usama’s portfolio</strong><span>Prepared response</span></div><div className="reply-content"><Reply topic={message.topic} /></div></div></section>)}<div className="follow-up-prompts" aria-label="Explore next">{topics.filter(topic => topic.id !== activeTopic).slice(0, 4).map(topic => <button key={topic.id} onClick={() => ask(topic.prompt, topic.id)}>{topic.label}<Icon name="external" /></button>)}</div></div>}
      </main>
